...
<head>
  ...
  <title><?= $notFound ? 'Collection not found' : $e($page['name']) ?> — PORTES</title>
  <?php if (!$notFound): ?>
  <meta name="description" content="The <?= $e($page['name']) ?> collection by PORTES — luxury architectural doors, made to measure." />
  <?php endif; ?>
...
<?php if ($notFound): ?>
  <section class="cd-404">
    ...
    <p>The collection you’re looking for doesn’t exist or is no longer available.</p>
    <a href="/door-showroom/collections" class="btn btn--gold">View all collections</a>
  </section>
<?php else: ?>
...
<section class="cd-hero<?= $page['hero'] ? '' : ' cd-hero--plain' ?>">
  ...
  <div class="cd-hero-inner">
    <nav class="cd-crumb" aria-label="Breadcrumb">
      <a href="/door-showroom">Home</a><span>/</span><a href="/door-showroom/collections">Collections</a><span>/</span><span><?= $e($page['name']) ?></span>
    </nav>
    <p class="cd-hero-label">The Collection</p>
    ...
    <div class="cd-hero-actions">
      <?php if (!empty($page['models'])): ?>
        <a href="#models" class="btn btn--gold btn--lg">Explore Products
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
        </a>
      <?php endif; ?>
      <a href="/door-showroom/configure?collection=<?= $e($page['slug']) ?>" class="btn btn--outline-light btn--lg">Configure a Door</a>
    </div>
  </div>
...
<section class="cd-features">
  <div class="sec-intro sec-intro--center reveal">
    ...
    <h2 class="cd-h2">Engineered<br /><em>into every door.</em></h2>
  </div>
...
<section class="cd-models" id="models">
  <div class="sec-intro reveal">
    <p class="eyebrow">Featured Models</p>
    <h2 class="cd-h2">Doors of the<br /><em><?= $e($page['name']) ?> collection.</em></h2>
  </div>
        ...
        <article class="cd-model reveal<?= $i ? ' reveal-d' . min($i, 3) : '' ?>">
          ...
          <div class="cd-model-body">
            ...
            <?php if (!empty($m['dimensions'])): ?>
              <div class="cd-model-spec"><span><em>Dimensions</em><?= $e($m['dimensions']) ?></span></div>
            <?php endif; ?>
            <?php if ($m['desc']): ?><p class="cd-model-desc"><?= $e($m['desc']) ?></p><?php endif; ?>
            <div class="cd-model-actions">
              <?php if (!empty($m['slug'])): ?>
                <a href="/door-showroom/product/<?= $e($m['slug']) ?>" class="btn btn--outline btn--sm">View Details</a>
              <?php endif; ?>
              <a href="/door-showroom/configure?collection=<?= $e($page['slug']) ?>" class="btn btn--gold btn--sm">Configure</a>
            </div>
          </div>
        </article>
      <?php endforeach; ?>
  ...
  <?php else: ?>
    <div class="cd-empty reveal">
      ...
      <h3>Products for this collection are coming soon.</h3>
      <p>Our atelier is preparing the <?= $e($page['name']) ?> models. In the meantime, you can configure a door to your exact specification.</p>
      <a href="/door-showroom/configure?collection=<?= $e($page['slug']) ?>" class="btn btn--gold">Configure a Door
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
      </a>
    </div>
  <?php endif; ?>
...
<section class="cd-finishes">
  <div class="sec-intro reveal">
    <p class="eyebrow">Colours Available</p>
    <h2 class="cd-h2">The <?= $e($page['name']) ?><br /><em>palette.</em></h2>
  </div>
...
<section class="quote<?= $page['hero'] ? '' : ' quote--plain' ?>" id="quote">
  ...
  <div class="quote-inner reveal">
    <h2 class="quote-title">Ready to create your<br /><em>perfect door?</em></h2>
    <p class="quote-sub">Open the configurator to make a <?= $e($page['name']) ?> door yours — material, finish, dimensions and hardware, with a personal quote.</p>
    <div class="quote-actions">
      <a href="/door-showroom/configure?collection=<?= $e($page['slug']) ?>" class="btn btn--gold btn--lg">Open Configurator
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
      </a>
