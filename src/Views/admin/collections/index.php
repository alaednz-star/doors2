...
<div class="page-header">
  <div>
    ...
    <p class="page-sub"><?= number_format($total) ?> <?= $total === 1 ? 'collection' : 'collections' ?> total</p>
  </div>
  <a href="/door-showroom/admin/collections/create" class="btn btn-primary">
...
<div class="table-panel">
  ...
  <div class="table-toolbar">
    <form method="GET" action="/door-showroom/admin/collections" class="search-form" id="searchForm">
      <div class="search-wrap">
        ...
        />
        <?php if ($q !== ''): ?>
          <a href="/door-showroom/admin/collections" class="search-clear" title="Clear search">
            <svg viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/></svg>
          </a>
        <?php endif; ?>
      </div>
      ...
      <tbody id="tableBody">
          ...
          <tr>
            <td colspan="8">
              <div class="table-empty">
                ...
                <?php if ($q !== ''): ?>
                  <p>No collections match "<?= $e($q) ?>".</p>
                  <a href="/door-showroom/admin/collections" class="btn btn-outline" style="margin-top:8px">Clear search</a>
                <?php else: ?>
                  <p>No collections yet.</p>
                  <a href="/door-showroom/admin/collections/create" class="btn btn-primary" style="margin-top:8px">Add your first collection</a>
                <?php endif; ?>
              </div>
            </td>
          </tr>
        <?php else: ?>
          <?php foreach ($collections as $col): ?>
            <tr id="row-<?= (int)$col['id'] ?>">
              <td class="td-muted"><?= (int)$col['id'] ?></td>
              <td><strong class="td-name"><?= $e($col['name']) ?></strong></td>
              <td><code class="td-slug"><?= $e($col['slug']) ?></code></td>
              <td class="td-desc">
                <?php if ($col['description']): ?>
                  <span title="<?= $e($col['description']) ?>"><?= $e(substr($col['description'], 0, 60)) ?><?= strlen($col['description']) > 60 ? '…' : '' ?></span>
                <?php else: ?>
                  <span class="td-muted">—</span>
                <?php endif; ?>
              </td>
              <td class="td-center"><?= (int)$col['display_order'] ?></td>
              <td class="td-center">
                <button class="toggle-status" data-id="<?= (int)$col['id'] ?>" data-active="<?= (int)$col['is_active'] ?>" title="Click to toggle status">
                  <span class="status-dot status-dot--<?= $col['is_active'] ? 'on' : 'off' ?>"></span>
                  <span class="status-label" id="status-label-<?= (int)$col['id'] ?>"><?= $col['is_active'] ? 'Active' : 'Inactive' ?></span>
                </button>
              </td>
              <td class="td-muted td-sm"><?= date('d M Y', strtotime($col['created_at'])) ?></td>
              <td class="td-actions">
                <a href="/door-showroom/admin/collections/<?= (int)$col['id'] ?>/edit" class="action-btn action-btn--edit" title="Edit">
                  <svg viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/></svg>
                </a>
                <button class="action-btn action-btn--delete" data-id="<?= (int)$col['id'] ?>" data-name="<?= $e($col['name']) ?>" title="Delete">
...
<div class="modal-backdrop" id="deleteModal" hidden>
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="modalTitle">
    ...
    <h3 id="modalTitle">Delete Collection</h3>
    <p>Are you sure you want to delete <strong id="modalItemName"></strong>? This action cannot be undone.</p>
...
(function () {
  ...
  var modalName = document.getElementById('modalItemName');
  ...
  modalBtn.addEventListener('click', function () {
    ...
    xhr('POST', '/door-showroom/admin/collections/' + deleteId + '/delete', function (data) {
      if (data.success) {
  ...
  document.querySelectorAll('.toggle-status').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      xhr('POST', '/door-showroom/admin/collections/' + id + '/toggle', function (data) {
        if (data.success) {
